...
export function TagBadge({ tag, count, size = "md" }: TagBadgeProps) {
  const sizeClass = size === "sm" ? "px-2.5 py-1" : size === "lg" ? "px-3.5 py-2" : "px-3 py-1.5";
  const textSize = size === "sm" ? "text-[0.68rem]" : size === "lg" ? "text-xs" : "text-[0.72rem]";
  ...
  return (
    ...
    >
      <Text
        className={`font-mono font-medium tracking-[0.01em] text-base-content/72 active:text-primary ${textSize}`}
      >
        #{tag}
