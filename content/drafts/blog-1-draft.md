...
description: "Finally rebuilt my blog for the 3rd time."
date: 2026-05-10
...
Media pipeline: images upload to R2 with content-hash dedup. A script rewrites local paths in markdown to CDN URLs. Generates a manifest with dimensions. OG images get proper width and height automatically. Zero manual URL work.

### CI/CD

Obviously GitHub Actions, since it's free for open source projects. Intelligent triggers: staging deploys on every push to master. Production only deploys when content or changelog files change. No wasted builds.
