...
*Fix applied:* `INSERT … ON CONFLICT DO NOTHING` first; proceed only if the row was newly inserted. Validate slug against `posts` table.
...
*Fix applied:* Rewrote `lib/rate-limit.ts` to use D1 `rate_limits` table with `COUNT(*)` + `INSERT` pattern.
...
*Fix applied:* Added `workers_dev = false` to both environments.
...
*Fix applied:* Set explicit `ALLOWED_ORIGIN` values in `wrangler.toml`.
...
*Fix applied:* Soft delete with status tracking. Queue consumer inserts into `blacklist` on permanent delivery failures.
...
*Fix applied:* `lib/tokens.ts` provides `deriveUnsubscribeToken()` and `hashToken()`. Subscribe stores hash; unsubscribe looks up by hash. Send endpoint derives tokens at enqueue time.
...
*Fix applied:* Added `dead_letter_queue = "newsletter-dlq"` to queue consumers. Consumer logs errors and falls back to `msg.ack()` after max retries (with blacklist insertion).
